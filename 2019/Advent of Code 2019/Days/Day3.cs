...
namespace AoC2019.Days
{
    public sealed class Day3 : DayBase<Dictionary<(int, int), int>[], int>
    {
        ...
        public override Dictionary<(int, int), int>[] ParseInput(IEnumerable<string> lines)
        {
        ...
        public override int RunPart1(Dictionary<(int, int), int>[] input)
        {
            return input[0].Keys.Intersect(input[1].Keys).Min(p => Math.Abs(p.Item1) + Math.Abs(p.Item2));
        }

        public override int RunPart2(Dictionary<(int, int), int>[] input)
        {
